...
function provisionServerProfile () {
	...
	eval $CLI_COMMAND $RHQ_OPTS -f "${WORKSPACE_WD}/cli/CLI/deployBundle.js ${BUNDLE_NAME} ${DESTINATION_NAME} ${DEPLOYMENT_DESC} JAVA_HOME=${JAVA_HOME} JBOSS_HOME=${JD_INSTALL_LOCATION}/${DEST_COMMON_SUFFIX} JBOSS_USER=jboss JBOSS_CONF_BASE=${JBOSS_BASE_CONF} JBOSS_PORTS_OFFSET=${PORT_SET}"
	newLine
}
